...
ggplot() +
  ...
  geom_pointrange(data = luebin %>% filter(!fg %in% "unclassified"), mapping = aes(x = bin_midpoint, y = median, ymin = q025, ymax = q975)) +
  facet_wrap(~ fg) +
...
ggplot() +
  ...
  geom_line(data = lue_fitted, mapping = aes(x = dbh, y = q025), linetype = 'dotted') +
  geom_line(data = lue_fitted, mapping = aes(x = dbh, y = q975), linetype = 'dotted') +
  facet_wrap(~ fg) +
  scale_x_log10(name = 'diameter (cm)') +
  scale_y_log10(name = parse(text = 'LUE~(kg~y^-1~W^-1)')) +
  scale_fill_gradient2(low = colors[3], mid = colors[2], high = colors[1], trans = 'log10') +
